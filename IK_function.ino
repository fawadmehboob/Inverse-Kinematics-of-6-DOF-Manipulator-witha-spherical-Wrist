...
void setup() {
  ...
  delay(1000);
  // Enter coordinates
    Serial.println("Enter Position and Orientation");

    Serial.print("Enter x: ");
    while (Serial.available() == 0) {}
    xg = Serial.readStringUntil('\n').toDouble();

    Serial.print("Enter y: ");
    while (Serial.available() == 0) {}
    yg = Serial.readStringUntil('\n').toDouble();

    Serial.print("Enter z: ");
    while (Serial.available() == 0) {}
    zg = Serial.readStringUntil('\n').toDouble();

    Serial.print("Enter yaw angle: ");
    while (Serial.available() == 0) {}
    yaw = Serial.readStringUntil('\n').toDouble();
    yaw = radians(yaw);
    Serial.print("Enter pitch angle: ");
    while (Serial.available() == 0) {}
    pitch = Serial.readStringUntil('\n').toDouble();
    pitch = radians(pitch);
    Serial.print("Enter roll angle: ");
    while (Serial.available() == 0) {}
    roll = Serial.readStringUntil('\n').toDouble();
    roll = radians(roll);
    IK(xg, yg, zg, yaw, pitch, roll, thetaC);
    for (int i=0; i<6; i++) {
...
void loop() {
  for (int i=0; i<6; i++){
    ...
    roll = Serial.readStringUntil('\n').toDouble();
    roll = radians(roll);
    if (xg <0 or zg<0){                                        // condition to home the manipulator in case of wrong input
